# Paths
# Fill this according to own setup
BACKGROUND_DIR = 'input_dir_cityscapes_rgbd/backgrounds/'
BACKGROUND_GLOB_STRING = '*leftImg8bit.png'
OBJECT_GLOB_STRING = '_crop.png'
POISSON_BLENDING_DIR = './pb'
SELECTED_LIST_FILE = 'input_dir_cityscapes_rgbd/selected.txt'
DISTRACTOR_LIST_FILE = 'input_dir_cityscapes_rgbd/neg_list.txt' 
DISTRACTOR_DIR = 'input_dir_cityscapes_rgbd/distractor_objects_dir/'
DISTRACTOR_GLOB_STRING = '*.jpg'
INVERTED_MASK = True # Set to true if white pixels represent background
ANNO_FILE_TXT='./output_dir_cityscapes_rgbd/annotations_txt/annotations.txt'

# Parameters for generator
NUMBER_OF_WORKERS = 1
BLENDING_LIST = ['none','poisson'] #'gaussian','motion'

# Parameters for images
MIN_NO_OF_OBJECTS = 1
MAX_NO_OF_OBJECTS = 1
MIN_NO_OF_DISTRACTOR_OBJECTS = 2
MAX_NO_OF_DISTRACTOR_OBJECTS = 4
WIDTH = 2048  #width of the synthesized image
HEIGHT = 1024  #height of the synthesized image
MAX_ATTEMPTS_TO_SYNTHESIZE = 20

# Parameters for objects in images
MIN_SCALE = 0.25 # min scale for scale augmentation
MAX_SCALE = 0.6 # max scale for scale augmentation
MAX_DEGREES = 30 # max rotation allowed during rotation augmentation
MAX_TRUNCATION_FRACTION = 0.25 # max fraction to be truncated = MAX_TRUNCACTION_FRACTION*(WIDTH/HEIGHT)
MAX_ALLOWED_IOU = 0.75 # IOU > MAX_ALLOWED_IOU is considered an occlusion
MIN_WIDTH = 6 # Minimum width of object to use for data generation
MIN_HEIGHT = 6 # Minimum height of object to use for data generation